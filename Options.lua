...
function OmniBar:ToggleLock(button)
	...
	self.options.args.bars.args[button.arg].args.lock.name = self.db.profile.bars[button.arg].locked and L["Unlock"] or
		L["Lock"]
end
	...
	for i = 0, MAX_CLASSES do
		spells[CLASS_SORT_ORDER_WITH_GENERAL[i]] = {
			...
			hidden = function(info)
				local total = #info
				local bar = info.options.args.bars.args[info[total - 2]]
				local class = info[total]
				return bar and next(bar.args.spells.args[class].args) == nil
			end,
			...
			if spell.class and spell.class == CLASS_SORT_ORDER_WITH_GENERAL[i] then
				...
				if spellName then
					...
					spells[CLASS_SORT_ORDER_WITH_GENERAL[i]].args[tostring(spellID)] = {
						...
						arg = spellID,
						desc = function()
							local duration = type(spell.duration) == "number" and spell.duration or
								spell.duration.default
							local spellDesc = descriptions[spellID] or ""
							-- Strip "Hold SHIFT" text as it's not functional in AceConfig tooltips
							spellDesc = spellDesc:gsub("\n*|c%x%x%x%x%x%x%x%xHold SHIFT for more information|r", "")
			...
			settings = {
				...
				args = {
					...
					adaptive = {
						...
						disabled = function()
							return (not self.db.profile.bars[key].showUnused) or
								self.db.profile.bars[key].trackUnit ~= "ENEMY"
						end,
						width = "normal",
						type = "toggle",
						order = 6,
...
local customSpellInfo = {
	...
	class = {
		...
		set = function(info, state)
			...
			OmniBar.db.global.cooldowns[spellId].duration = {
				default = OmniBar.db.global.cooldowns[spellId].duration
					.default
			}
			OmniBar.db.global.cooldowns[spellId][option] = state
			OmniBar:OnEnable() -- to refresh the bar spells tab
	...
	spellId = {
		...
		set = function(info, state, data)
			...
			if spellId and name then
				OmniBar.db.global.cooldowns[spellId] = data or addon.Cooldowns[spellId] or
					{ custom = true, duration = { default = 30 }, class = "GENERAL" }
				...
				if OmniBar.db.global.cooldowns[spellId].parent then
					...
					OmniBar.db.global.cooldowns[spellId] = OmniBar:CopyCooldown(addon.Cooldowns
						[OmniBar.db.global.cooldowns[spellId].parent])
				...
				if type(OmniBar.db.global.cooldowns[spellId].duration) == "number" then
					OmniBar.db.global.cooldowns[spellId].duration = {
						default = OmniBar.db.global.cooldowns[spellId]
							.duration
					}
				end
				OmniBar:AddCustomSpells()
		...
		args = {
			vers = {
				...
				name = "|cffffd700 " .. L["Version"] ..
					"|r " .. self.version.string .. "|cffffd700, Based on OmniBar|r v29\n",
				cmdHidden = true
			},
			desc = {
				order = 2,
...
-- [Shim] SetClipsChildren
if (not exportEditBox.frame.SetClipsChildren) then
	exportEditBox.frame.SetClipsChildren = function() end
end
...
-- [Shim] SetClipsChildren
if (not importEditBox.frame.SetClipsChildren) then
	importEditBox.frame.SetClipsChildren = function() end
end
