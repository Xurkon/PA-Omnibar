...
function orderByTimeLeft()
	compactArray(_G["OmniBar"].active)
	table.sort(_G["OmniBar"].active, function(x, y)
		if not x or not y then return false end
		local xFinish = x.cooldown and x.cooldown.finish
		local yFinish = y.cooldown and y.cooldown.finish
		if xFinish and yFinish then
...
function OmniBar_RefreshIcons(self)
	-- Stop moving if we are dragging
