...
if (not C_Timer) then
	...
	WaitFrame:SetScript("OnUpdate", function(self, elapsed)
		for i = #Ticker, 1, -1 do
			local Tick = Ticker[i]
			if (Tick._Cancelled) then
				table.remove(Ticker, i)
			elseif (Tick._Delay > elapsed) then
				Tick._Delay = Tick._Delay - elapsed
			else
				Tick._Handler()
				table.remove(Ticker, i)
			end
	...
	function C_Timer.After(Duration, Callback)
		table.insert(Ticker, { _Delay = Duration, _Handler = Callback })
	end
end
...
if (not C_PvP) then
	C_PvP = {}
	function C_PvP.IsRatedBattleground() return false end
end
...
local function BuildSpellNameTable()
	wipe(SPELL_ID_BY_NAME)
	for spellID, data in pairs(addon.Cooldowns) do
		-- Only map if we have a valid ID
		if type(spellID) == "number" then
			local name = GetSpellInfo(spellID)
			if name then
				SPELL_ID_BY_NAME[name] = spellID
			end
		end
	end
end
...
function OmniBar:OnInitialize()
	...
	-- Set version
	local version, major, minor = GetAddOnMetadata(addonName, "Version") or "", 0, 0
...
function OmniBar:Initialize(key, name)
	...
	f.MAPoint = true -- MoveAnything: Unanchoring Fix
...
function OmniBar_CreateIcon(self)
	...
	-- [Shim] Missing Cooldown Methods in 3.3.5a
	if (f.cooldown) then
		if (not f.cooldown.SetHideCountdownNumbers) then
			f.cooldown.SetHideCountdownNumbers = function(self, hide)
				self.noCooldownCount = hide
			end
		end

		-- Proactively shim other methods that might be missing
		local methods = { "SetSwipeColor", "SetDrawEdge", "SetDrawBling", "SetSwipeTexture", "SetEdgeTexture",
			"SetBlingTexture" }
		for _, method in ipairs(methods) do
			if (not f.cooldown[method]) then
				f.cooldown[method] = function() end
			end
		end
	end
end
...
function OmniBar:AddSpellCast(event, sourceGUID, sourceName, sourceFlags, spellID, serverTime, customDuration)
	...
	if self.spellCasts[name] and
		...
		(self.spellCasts[name][spellID].serverTime == serverTime or ((customDuration or addon.Cooldowns[spellID].eventThrottle) and self.spellCasts[name][spellID].expires > (now + (customDuration or 0))))
	then
		return
	end
...
function OmniBar_CooldownFinish(self, force)
	...
	-- Halt reverse finish timer, if main CD preemptively hidden.
	if self.___CooldownReverseFinish:GetProgress() < 1 then
		self.___CooldownReverseFinish:Stop()
	end
...
function OmniBar_StartCooldown(self, icon, start)
	...
	-- Reverse CD Finish (Hide)
	icon.cooldown.___CooldownReverseFinish:Stop()
	icon.cooldown.___CooldownReverseFinish.duration:SetDuration((icon.duration + .1) - (GetTime() - start))
	icon.cooldown.___CooldownReverseFinish:Play()
end
